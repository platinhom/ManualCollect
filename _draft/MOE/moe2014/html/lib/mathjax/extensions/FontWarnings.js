...
(function (HUB,HTML) {
  ...
  if (!HTML.Cookie.Get("fontWarn").warned) {
    ...
    HUB.Startup.signal.Interest(function (message) {
      // This block of code was added by CCG on 2014-08-15 b/c IE7-8 were failing on message.match in this function.
      if (gman_usingOldIE != undefined) {
        if (gman_usingOldIE) {
          if (!HTML.Cookie.Get("fontWarn").warned) {
            alert ("You are using an old version of Internet Explorer, which will not display mathematics nicely.\n\n"
                 + "Please consider using Firefox or Chrome instead, to better display the mathematics.\n");
            HTML.Cookie.Set("fontWarn",{warned:true});
          }
          return;
        }
